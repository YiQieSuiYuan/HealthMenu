...
//时间戳 → 北京时间
+(NSString *)timedsampToDate:(NSString *)string
{
    //时间戳转时间
    NSDate *confromTimesp = [NSDate dateWithTimeIntervalSince1970:[string doubleValue]];
    NSString *confromTimespStr = [self dateToBJTime:confromTimesp];
...
+ (NSString *)pingjieStringFromArray:(NSArray *)array string:(NSString *)string
{
    ...
    for (NSString *obj in array)
    {
        ...
        {
            retString = [retString stringByAppendingString:[NSString stringWithFormat:@",%@",obj]];
        }
    }
...
//newVersion新版本号
//oldVersion旧版本号
+ (NSInteger) compareVersion:(NSString*) newVersion andOldVersion:(NSString*) oldVersion
{
    //按.分割
    NSArray *newArray =[newVersion componentsSeparatedByString:NSLocalizedString(@".", nil)];
    NSArray *oldArray =[oldVersion componentsSeparatedByString:NSLocalizedString(@".", nil)];
    //格式化版本号
    NSString *newString = [NSString stringWithFormat:@"%02d%02d%02d",[newArray[0]intValue],[newArray[1]intValue],[newArray[2]intValue]];
    NSString *oldString = [NSString stringWithFormat:@"%02d%02d%02d",[oldArray[0]intValue],[oldArray[1]intValue],[oldArray[2]intValue]];
    //转为数字进行比较
    int newIntValue = [newString intValue];
    int oldIntValue = [oldString intValue];
